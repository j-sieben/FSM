begin
  ...
  pit_admin.merge_message(
    ...
    p_pms_pmg_name => 'FSM',
    p_pms_text => q'^Das Ereignis "#1#" wurde deklariert, aber nicht verwendet.^',
    p_pms_description => q'^Ereignisse, die nicht verwendet werden, sollten nicht deklariert werden.^',
    p_pms_pse_id => 30,
    p_pms_pml_name => 'GERMAN',
    p_error_number => -20000
  );
  ...
  pit_admin.merge_message(
    ...
    p_pms_pmg_name => 'FSM',
    p_pms_text => q'^Monitorstatus der FSM #1# wurde von #2# auf #3# geändert.^',
    p_pms_description => q'^Die zentrale FSM-Überwachung hat einen neuen persistierten Monitorstatus erkannt.^',
    p_pms_pse_id => 60,
    p_pms_pml_name => 'GERMAN',
    p_error_number => null
  );
