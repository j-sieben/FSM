create or replace package body fsm_pkg 
as
  ...
  procedure persist_retry(
    ...
    p_retry_schedule in fsm_object.fsm_retry_schedule%type)
  as
  begin
    pit.enter_detailed(
      p_params => msg_params(
  ...
  procedure retry(
    ...
    p_fev_id in fsm_event.fev_id%type)
  as
    cursor fsm_cur (p_fsm_id in fsm_object.fsm_id%type) is
    ...
    l_validity fsm_object.fsm_validity%type;
  begin
    pit.enter_mandatory(
      p_params => msg_params(
                    msg_param('p_fsm', 'opaque'),
  ...
  function allows_event(
    p_fsm in out nocopy fsm_type,
    p_fev_id fsm_event.fev_id%type)
    return boolean
  as
    l_result boolean;
