...
with next_events as
       (select fsm_id, fev_id, fev_command_label, fev_description, ftr_required_role, fev_button_highlight, fev_button_icon, fev_confirm_message
          from fsm_objects_v
          join fsm_events_v
            on instr(fsm_fev_list, fev_id) > 0
           and fcl_id = fev_fcl_id
          join fsm_transitions
            on fcl_id = ftr_fcl_id
           and fst_id = ftr_fst_id
           and fev_id = ftr_fev_id
         where fev_raised_by_user = &C_TRUE.
       )
